pipeline{
    ...
    environment {  
        ...
        DEPLOTMENT_PATH = 'K8S/deployment.yaml'                      // path to deployment.yml in GitHub repo
        STATEFULSET_PATH = 'K8S/statefulset.yaml'                   // path to the statefulset.yml in GitHub repo
            ...
            steps {
                ...
                withAWS(credentials: "${AWS_CREDENTIALS_ID}"){
                    withCredentials([file(credentialsId: "${KUBECONFIG_ID}", variable: 'KUBECONFIG')]) {
                        sh "kubectl apply -f K8S"   // 'K8S' is a directory contains all kubernetes manifists
                    }                          
                }
            }
        }
